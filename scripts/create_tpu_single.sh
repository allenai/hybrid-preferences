...
gcloud alpha compute tpus tpu-vm ssh $tpu_name --zone=$zone --project=ai2-tpu --worker=all --command="pip install libtpu-nightly==0.1.dev20230622 -f https://storage.googleapis.com/jax-releases/libtpu_releases.html"
gcloud alpha compute tpus tpu-vm ssh $tpu_name --zone=$zone --project=ai2-tpu --worker=all --command="pip install jaxlib[tpu]==0.4.13 -f https://storage.googleapis.com/jax-releases/libtpu_releases.html --no-deps"
gcloud alpha compute tpus tpu-vm ssh $tpu_name --zone=$zone --project=ai2-tpu --worker=all --command="pip install jax==0.4.13 -f https://storage.googleapis.com/jax-releases/libtpu_releases.html --no-deps"
